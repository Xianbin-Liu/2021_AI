...
class KNNRegression(KNN):
    ...
    def predict(self, X, k=1, lossType=2):
        ...
        dist_K = np.argsort(dist)[:,:self.K]  # the least K smaples
        # Y = np.argmax(dist_K, axis=1)
        Y_K = self.y[dist_K]        # shape : <N, K, C>
        for i in range(len(Y_K)):
            dist_k_i = dist[i][dist_K[i]].reshape(self.K,1)
            Y.append(np.sum(Y_K[i] / dist_k_i, axis=0) / np.sum(1/dist_k_i))   # of shape <K, C>  Y_K[i][k] / distance_k
...
def main():
    folder = '/Users/liuxb/2021_au/al_rao/lab/lab1/lab1_data/regression_dataset'
    savepath = '/Users/liuxb/2021_au/al_rao/lab/lab1/result/regression_simple_test.csv'

    train_file = 'train_set.csv'
    validation_file = 'validation_set.csv'
    test_file = 'regression_simple_test.csv'
    ...
    test_set, content = readtestfile(folder+'/'+test_file, vocab, idf)
